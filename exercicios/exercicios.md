...
```sh
$ cd redis
$ docker build -t ricardo85x/redis:devops .
$ docker run -d --name redis -p 6379:6379 ricardo85x/redis:devops
$ docker ps
$ docker logs redis
```
...
```sh
$ cd ../node
$ docker build -t ricardo85x/node:devops .
```
Agora iremos rodar a imagem do node, fazendo a ligação dela com o container do Redis.
```sh
$ docker run -d --name node -p 8080:8080 --link redis ricardo85x/node:devops
$ docker ps 
$ docker logs node
```
...
```sh
$ cd ../nginx
$ docker build -t ricardo85x/nginx:devops .
```
Criando o container do nginx a partir da imagem e fazendo a ligação com o container do Node
```sh
$ docker run -d --name nginx -p 80:80 --link node ricardo85x/nginx:devops
$ docker ps
```
